...
function typeLabel(type: InboxTypeConfig['type']): string {
  return type === 'solicitud' ? 'Solicitud' : 'Tarea';
}

function typeVariant(type: InboxTypeConfig['type']): 'info' | 'neutral' {
  return type === 'solicitud' ? 'info' : 'neutral';
}
</script>
...
<template>
  <div class="flex flex-col gap-2" data-testid="inbox-types-list">
    ...
    <ul class="flex flex-col gap-2" role="list">
      <li v-for="t in types" :key="t.concept">
        <button
          type="button"
          ...
          :data-testid="`inbox-type-${t.concept}`"
          @click="emit('select', t)"
        >
