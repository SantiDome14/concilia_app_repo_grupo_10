...
**Mutation persistence pattern.** Pages may keep a local `ref` mirror of query data for ergonomic in-place mutations (e.g. when delegating to the manifest engine). Every user-initiated mutation MUST also call the matching API module function so the MSW seed reflects the change and a refresh preserves state. The pattern: query → mirror in `ref` via `watch`, mutate locally for UX, fire-and-forget `update<Domain>(id, record)` to persist. See `src/pages/Inbox.vue` for the canonical example.
