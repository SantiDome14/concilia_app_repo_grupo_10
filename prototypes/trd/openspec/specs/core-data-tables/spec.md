...
On the first activation of the Tablero view per session for a multi-axis module, a `<KanbanAxisDialog>` MUST open listing the available axes (with description and a `read-only` chip when `readOnly: true`). The user's choice MUST persist in session storage scoped per module. The board header SHALL display the active axis label and a `Cambiar eje` button that re-opens the dialog. The board MUST resolve and mutate the state via `_resolveField(record, stateField)` and `_setField(record, stateField, value)` so that dot-paths work (`'fin.imput'` reads/writes `record.fin.imput`).
...
- **THEN** the kanban renders with the `imputacion` axis active without re-opening the dialog, and the board header shows `Organizando por: Imputación` plus a `Cambiar eje` button
...
- **GIVEN** a module attempts to register `axes: { a: { stateField: 'state', ...}, b: { stateField: 'state', ...} }`
- **WHEN** the framework processes the module registration
- **THEN** registration fails with a developer-facing error naming the duplicate `stateField`, and the module MUST refactor to a single axis
