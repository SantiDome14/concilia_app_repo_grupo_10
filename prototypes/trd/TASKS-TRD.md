...
| 4 | `add-trd-quote-ccc` | Modal | — | trd-discovery §5.1 | **Shipped** | v1: modal con 3 currencies (origen/intermedia/destino), 2 rates, live calc de montos intermedio + destino + TC consolidado. POST /quotes/ccc genera ccc_group_id único + 3 quotes legs (leg1 orig→middle, leg2 middle→dest, leg3 consolidado orig→dest). Activity events por leg en MSW handler. Badge `CCC` info-variant en filas con ccc_group_id (visibilidad sin agregar columnas). **Diferido**: `extend-trd-quote-ccc-merged-rows` (collapse de 3 legs en 1 row del master), `extend-trd-quote-ccc-detail-dialog` (vista unificada 3-leg). |
...
- **2026-05-27 — session 12:** Shipped `add-trd-quote-ccc` (priority 4) — **cerramos Stage 1**. CreateCCCQuoteModal con 3 currencies + 2 rates + live calc de middle/destination/composite-rate. POST /quotes/ccc handler crea 3 legs con ccc_group_id compartido. Badge `CCC` info-variant en las filas con ccc_group_id (visibilidad cheap sin tocar columnas). CTA "Nueva CCC" secondary junto a "Nueva cotización" primary. 6 tests nuevos (4 api + 2 page); 572/572 total. **Stage 1 completo** — solo las priorities 9-13 quedan (RFQ Gateway / Notificaciones / Home Exposición, fuera de scope per session 1 decision).
