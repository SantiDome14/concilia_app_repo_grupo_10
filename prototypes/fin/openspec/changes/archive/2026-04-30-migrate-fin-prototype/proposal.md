...
- **Affected code paths:** all of the FIN domain (`src/manifests/`, `src/pages/`, `src/router/routes.ts`, `src/types/fin.ts`, `src/mocks/fin/`, `src/plugins/{manifests,catalogs}.ts`, branding) PLUS framework code that ports back to `_core-template/`: `src/components/{settings,modals,kanban,manifest,ui}/`, `src/composables/useSettingsDialog.ts`, `src/stores/preferences.ts`, `src/lib/{kanban,manifest}/`, `src/plugins/auth0.ts`, `src/styles/globals.css`, `src/App.vue`, `src/components/layout/Sidebar.vue`.
