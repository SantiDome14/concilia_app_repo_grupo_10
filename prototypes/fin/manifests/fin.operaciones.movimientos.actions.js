/* ───────────────────────────────────────────────────────────────────
   Manifest · FIN.Operaciones · Movimientos (caso piloto principal)
   ───────────────────────────────────────────────────────────────────
   Cubre la imputación financiera de movimientos crudos del módulo
   Operaciones (vista de OPS con lente Finanzas). Cada tipo de
   movimiento declara qué campos son requeridos para llegar a
   estado `IMP` (imputado) — ver `required_by_type`.

   Lifecycle del registro:
     · PEND → todos los target_fields requeridos están null
     · PARC → al menos uno asignado, falta alguno
     · IMP  → todos asignados (sale del flujo de imputación)

   El eje `fin.imput` del Kanban legacy del módulo (declarado en
   MOD_REG.mov.axes) se mapea 1:1 a este lifecycle: el motor del
   manifest (MFcomputeImputation) escribe en `record.fin.imput`
   los valores PEND/PARC/IMP automáticamente después de cada
   confirm.
...
window.ACTION_MANIFEST["fin.operaciones.movimientos"] = {
  "app": "fin",
  "module": "operaciones",
  ...
  "actions": [
    {
      "id": "fin.operaciones.movimientos.imputacion.asignar_estructura",
      "dimension": "imputacion",
      "label": "Asignar Estructura",
      "description": "Imputá el movimiento a una de las sociedades del grupo.",
      "icon": "building",
      "target_field": "fin.sociedad_id",
      "show_when": { "record_type_in": ["DEPOSIT","WITHDRAWAL","COLLECTOR_IN","COLLECTOR_OUT","SWAP_OUT","SWAP_IN","FEE","TAX","REBATE","ADDITION","TRANSFER_OUT","TRANSFER_IN"] },
      ...
      "capabilities": { "required_role_any_of": ["OPS_OFFICER","ADMIN_OPS","ADMIN_FIN","ANALISTA_CONTABLE","ADMIN"] },
      "dialog": {
        "title": "Asignar Estructura",
        "description": "Identificá la sociedad del grupo a la que pertenece el movimiento.",
        "fields": [
          { "id": "fin.sociedad_id", "label": "Sociedad", "type": "lookup",
            "catalog": "framework.sociedades", "required": true,
    ...
    {
      "id": "fin.operaciones.movimientos.imputacion.asignar_banco_cuenta",
      "dimension": "imputacion",
      ...
      "disable_tag": "Asignada",
      "prerequisites": [
        { "field": "fin.sociedad_id", "message": "Asigná Estructura primero" }
      ],
      "capabilities": { "required_role_any_of": ["OPS_OFFICER","ADMIN_OPS","ADMIN_FIN","ANALISTA_CONTABLE","ADMIN"] },
      "dialog": {
        "title": "Asignar Banco y Cuenta",
        "description": "Elegí la cuenta física de la sociedad. La lista se filtra por la Estructura asignada.",
        "fields": [
          { "id": "fin.cuenta_id", "label": "Cuenta", "type": "lookup",
    ...
    {
      "id": "fin.operaciones.movimientos.imputacion.asignar_cliente",
      "dimension": "imputacion",
      "label": "Asignar Cliente",
      "description": "Identificá al cliente de origen del movimiento.",
      "icon": "user-plus",
      "target_field": "fin.cliente_id",
      "show_when": { "record_type_in": ["DEPOSIT","WITHDRAWAL","COLLECTOR_IN","COLLECTOR_OUT","SWAP_OUT","SWAP_IN"] },
      ...
      "dialog": {
        ...
        "fields": [
          { "id": "fin.cliente_id", "label": "Cliente", "type": "lookup",
          ...
          { "id": "fin.cliente_imputation_note", "label": "Nota de imputación (opcional)",
    ...
    {
      "id": "fin.operaciones.movimientos.imputacion.asignar_proveedor",
      "dimension": "imputacion",
      ...
      "disable_tag": "Asignado",
      "capabilities": { "required_role_any_of": ["ADMIN_FIN","ANALISTA_CONTABLE","ADMIN"] },
      "dialog": {
        "title": "Asignar Proveedor",
        "description": "Elegí el proveedor que cobró la comisión.",
        "fields": [
          { "id": "fin.proveedor_id", "label": "Proveedor", "type": "lookup",
            "catalog": "fin.proveedores", "required": true, "placeholder": "Buscar proveedor..." }
        ],
        "confirm_label": "Asignar"
      },
      "on_confirm": {
    ...
    {
      "id": "fin.operaciones.movimientos.imputacion.asignar_partner",
      "dimension": "imputacion",
      ...
      "dialog": {
        ...
        "fields": [
          { "id": "fin.partner_id", "label": "Partner", "type": "lookup",
            "catalog": "fin.partners", "required": true, "placeholder": "Buscar partner..." }
        ],
        "confirm_label": "Asignar"
      },
      "on_confirm": {
    ...
    {
      "id": "fin.operaciones.movimientos.imputacion.asignar_banco_exchange",
      "dimension": "imputacion",
      ...
      "dialog": {
        ...
        "fields": [
          { "id": "fin.banco_id", "label": "Banco / Exchange", "type": "lookup",
            "catalog": "framework.bancos_exchanges", "required": true,
    ...
    {
      "id": "fin.operaciones.movimientos.imputacion.imputar_cuenta_contable",
      "dimension": "imputacion",
      ...
      "dialog": {
        ...
        "fields": [
          { "id": "fin.cuenta_contable_id", "label": "Cuenta contable", "type": "text", "required": true }
        ]
      },
      "on_confirm": {
    ...
    {
      "id": "fin.operaciones.movimientos.governance.marcar_intercompany",
      ...
      "capabilities": { "required_role_any_of": ["ADMIN_FIN","ADMIN"] },
      "dialog": {
        "title": "Marcar como Intercompany",
        "description": "Esta acción genera un movimiento espejo en la sociedad contraparte y los liga como intercompany. Audit log explícito.",
        "fields": [
          { "id": "fin.intercompany_counterparty_sociedad_id", "label": "Sociedad contraparte",
            "type": "lookup", "catalog": "framework.sociedades", "required": true,
            "placeholder": "Buscar sociedad contraparte..." },
          { "id": "fin.intercompany_note", "label": "Nota (opcional)",
            "type": "textarea", "required": false, "max_length": 280 }
        ],
        "confirm_label": "Marcar y generar espejo"
      },
      "on_confirm": {
        "update_fields": ["fin.intercompany_counterparty_sociedad_id", "fin.intercompany_note"],
    ...
    {
      "id": "fin.operaciones.movimientos.conciliacion.marcar_conciliado",
      ...
      "capabilities": { "required_role_any_of": ["OPS_OFFICER","FINANCE","ADMIN_FIN","ANALISTA_CONTABLE","ADMIN"] },
      "dialog": {
        "title": "Marcar movimiento como Conciliado",
        "description": "Confirmá que el movimiento corresponde al asiento del extracto bancario. La acción registra la decisión en el audit log.",
        "fields": [
          { "id": "fin.conc_note", "label": "Nota de conciliación (opcional)",
            "type": "textarea", "required": false, "max_length": 280 }
        ],
