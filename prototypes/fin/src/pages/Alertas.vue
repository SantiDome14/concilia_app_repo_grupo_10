...
// ─── vue-query is the source of truth ────────────────────────────────
...
const alertasQuery = useQuery({ queryKey: ALERTAS_KEY, queryFn: listAlertas });
...
const filteredAlertas = computed<Alerta[]>(() => {
  ...
  return alertas.value.filter((a) => {
    if (filterType.value && a.concept !== filterType.value) return false;
...
// ─── Drawer — derive from cache for optimistic updates to flow in ────
...
onMounted(() => {
  ...
  alertasMod.registerDispatcher({
    update: (recordId, patch) => {
      updateMutation.mutate({ id: recordId, patch: patch as Partial<Alerta> });
    },
    create: () => {
