...
const props = defineProps<{
  ...
  tooltip?: boolean;
  /** Stacked vs grouped (default `'grouped'`). */
...
const yAccessors = computed(() => resolvedSeries.value.map((s) => s.accessor));
...
<template>
  ...
  <div
    v-else
    class="h-full w-full"
    :aria-label="props.description ?? props.title ?? 'Gráfico de barras'"
  >
    <VisXYContainer
      :data="props.data"
