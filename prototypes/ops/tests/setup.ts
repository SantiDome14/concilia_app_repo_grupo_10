...
import { resetBanksAccountsSeed } from '@/mocks/seed/banksAccounts';
...
import { resetInstructionsSeed } from '@/mocks/seed/instructions';
import { resetMovementsSeed } from '@/mocks/seed/movements';
import { resetPspSeed } from '@/mocks/seed/psp';
import { resetReportsSeed } from '@/mocks/seed/reports';
import { resetSolicitudesSeed } from '@/mocks/seed/solicitudes';
import { resetTradesSeed } from '@/mocks/seed/trades';
...
// 3. Per-domain seed resets are registered incrementally as each
//    `src/mocks/seed/<domain>.ts` lands during the OPS migration.
// ════════════════════════════════════════════════════════════════════
...
afterEach(() => {
  ...
  queryClient.clear();
  resetAlertasSeed();
  resetBanksAccountsSeed();
  resetClientsSeed();
  resetDashboardKpisSeed();
  resetExamplesSeed();
  resetInstructionsSeed();
  resetMovementsSeed();
  resetPspSeed();
  resetReportsSeed();
