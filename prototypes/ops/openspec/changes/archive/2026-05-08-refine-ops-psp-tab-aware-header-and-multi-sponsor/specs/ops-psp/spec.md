...
**Reason for removal:** The page-header right-actions slot is reserved for the canonical `<ViewToggle>` (3 vistas) + main CTA per active tab. The Coinag health chip MUST move out of the page header into the Posición tree per-sponsor row so it is co-located with the sponsor it describes. Replaced by the ADDED requirement of the same shape with the new mounting location.
...
**Reason for removal:** The Cuentas tab's page-header main CTA is replaced by `Crear Cuenta` (a real account-creation surface — placeholder until wired). The `Habilitar cuenta` CTA was the previous page-header trigger for the whitelist modal; that mounting point is retired. The `<WhitelistAccountModal>` component is preserved in the codebase for re-cabling from a drawer-context (e.g. inside the SWIFT transactions drawer when an operator inspects a counterparty account); a follow-up `extend-ops-psp-whitelist-from-drawer` change re-cables it. The cross-capability composition contract with `ops-clients` (direct reuse, no duplication) is preserved by the component itself — only the page-header CTA is removed.
