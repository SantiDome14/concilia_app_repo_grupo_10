...
### Framework foundations (`prototypes/_core-template`)
...
- Template-only modules (`ModuloA/B/C`, playground) removed from derived apps; stay in `_core-template` only (Pattern 12).
...
- 16 patterns + 19 antipatterns codified in `prototypes/_core-template/MIGRATION-PLAYBOOK.md`.
...
- This PR is the migration cumulative — each commit is independently reviewable but the unit of value is the whole branch. Recommended review path: skim `prototypes/_core-template/MIGRATION-PLAYBOOK.md` first (the patterns + antipatterns capture the reasoning behind every architectural decision), then `prototypes/ops/MIGRATION-NOTES.md` (OPS-specific decisions), then the spec changes under `prototypes/ops/openspec/changes/archive/2026-05-08-*/`.
