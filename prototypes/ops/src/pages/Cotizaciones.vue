...
import { listQuotes } from '@/ops/cotizaciones/api';
import CotizacionesFilters from '@/ops/cotizaciones/CotizacionesFilters.vue';
import CotizacionesTable from '@/ops/cotizaciones/CotizacionesTable.vue';
import type { QuotesView } from '@/ops/cotizaciones/types';

// ════════════════════════════════════════════════════════════════════
// Cotizaciones page — implements ops-cotizaciones Requirements 1-5.
// Type-A master list at /cotizaciones with sub-toggle Active / Historic.
...
const quotesQuery = useQuery({
  queryKey: computed(() => ['ops', 'cotizaciones', 'list', quoteQueryParams.value] as const),
...
<template>
  <div class="flex flex-col gap-5 p-6">
    <div class="flex items-center justify-between gap-4">
      <div>
        <h1 class="text-xl font-bold text-t-1">Cotizaciones</h1>
    ...
    />
    <CotizacionesTable :rows="quotes" :is-loading="quotesQuery.isPending.value" />
  </div>
</template>
