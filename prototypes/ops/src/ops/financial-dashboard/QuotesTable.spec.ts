...
describe('QuotesTable', () => {
  ...
  it('renders BUY operations in success tone, SELL in danger', () => {
    const wBuy = mount(QuotesTable, { props: { rows: [makeQuote({ operation: 'BUY' })], isLoading: false } });
    expect(wBuy.html()).toContain('text-success');

    const wSell = mount(QuotesTable, { props: { rows: [makeQuote({ operation: 'SELL' })], isLoading: false } });
  ...
  it('renders the read-only status as a Badge (no clickable button)', () => {
    const w = mount(QuotesTable, { props: { rows: [makeQuote({ status: 'ACCEPTED' })], isLoading: false } });
    const buttons = w.findAll('tbody button');
    expect(buttons).toHaveLength(0);
  });

  it('renders different status badges with the correct semantic tone', () => {
    const wAccepted = mount(QuotesTable, { props: { rows: [makeQuote({ status: 'ACCEPTED' })], isLoading: false } });
    expect(wAccepted.html()).toContain('text-success');

    const wExpired = mount(QuotesTable, { props: { rows: [makeQuote({ status: 'EXPIRED' })], isLoading: false } });
    expect(wExpired.html()).toContain('text-danger');

    const wPending = mount(QuotesTable, { props: { rows: [makeQuote({ status: 'PENDING' })], isLoading: false } });
