...
import CotizacionesTable from './CotizacionesTable.vue';
...
describe('CotizacionesTable', () => {
  it('renders skeleton rows while loading', () => {
    const w = mount(CotizacionesTable, { props: { rows: [], isLoading: true } });
    expect(w.findAll('tbody tr').length).toBe(5);
  });

  it('renders the empty state when there are no quotes', () => {
    const w = mount(CotizacionesTable, { props: { rows: [], isLoading: false } });
    expect(w.text()).toContain('Sin quotes');
  });

  it('renders the canonical 9-column header set', () => {
    const w = mount(CotizacionesTable, { props: { rows: [makeQuote()], isLoading: false } });
  ...
  it('renders BUY operations in success tone, SELL in danger', () => {
    const wBuy = mount(CotizacionesTable, { props: { rows: [makeQuote({ operation: 'BUY' })], isLoading: false } });
    expect(wBuy.html()).toContain('text-success');

    const wSell = mount(CotizacionesTable, { props: { rows: [makeQuote({ operation: 'SELL' })], isLoading: false } });
  ...
  it('renders the read-only status as a Badge (no clickable button)', () => {
    const w = mount(CotizacionesTable, { props: { rows: [makeQuote({ status: 'ACCEPTED' })], isLoading: false } });
    const buttons = w.findAll('tbody button');
    expect(buttons).toHaveLength(0);
  });

  it('renders different status badges with the correct semantic tone', () => {
    const wAccepted = mount(CotizacionesTable, { props: { rows: [makeQuote({ status: 'ACCEPTED' })], isLoading: false } });
    expect(wAccepted.html()).toContain('text-success');

    const wExpired = mount(CotizacionesTable, { props: { rows: [makeQuote({ status: 'EXPIRED' })], isLoading: false } });
    expect(wExpired.html()).toContain('text-danger');

    const wPending = mount(CotizacionesTable, { props: { rows: [makeQuote({ status: 'PENDING' })], isLoading: false } });
