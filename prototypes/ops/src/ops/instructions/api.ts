...
// save), 8 (delete) of the spec. The shared `apiClient` from
// `core-api-layer` carries the bearer token via setAccessTokenGetter.
// ════════════════════════════════════════════════════════════════════

const ENDPOINTS = {
  list: '/instruction',
  detail: (id: InstructionId): string => `/instruction/${id}`,
  attributes: (id: InstructionId): string => `/instruction-attribute/instruction/${id}`,
...
export async function listInstructions(
  ...
  const response = await apiClient.get<PaginatedResponse<Instruction>>(ENDPOINTS.list, {
    params,
  });
...
export async function getInstruction(
  ...
  const [instruction, attributes] = await Promise.all([
    apiClient.get<Instruction>(ENDPOINTS.detail(id)).then((r) => r.data),
    apiClient
      .get<InstructionAttribute[]>(ENDPOINTS.attributes(id))
      .then((r) => r.data ?? []),
  ]);
...
export async function createInstructionRecord(
  ...
  const response = await apiClient.post<Instruction>(ENDPOINTS.list, {
    name: data.name,
...
export async function saveInstructionAttributes(
  ...
  const response = await apiClient.post<InstructionAttribute[]>(ENDPOINTS.saveAttributes, {
    instruction_id: instructionId,
    attributes: payload,
  });
  return response.data ?? [];
...
export async function createInstructionWithAttributes(
  ...
  // Phase B (if there are no attributes, treat as success).
...
export async function updateInstructionWithAttributes(
  ...
  // Phase A — PUT /instruction/:id.
  let updated: Instruction;
  try {
    const response = await apiClient.put<Instruction>(ENDPOINTS.detail(id), {
      name: data.name,
      currency_id: data.currency_id,
      description: data.description || null,
    });
...
function extractPhaseAError(e: unknown): { message: string; field?: keyof InstructionFormData } {
