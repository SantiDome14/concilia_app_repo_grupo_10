---
name: Operatoria del rulo — Pipeline ARS → Stablecoins
features: [COMMON]
status: Concluida
...
propagates_to:
  - features/common/operatoria-rulo.md
  - entities/haz-pagos.md
  - entities/circuit-pay.md
---
...
Documentar el pipeline ARS → Stablecoins (rulo) como fuente de verdad
operativa del framework: el caso canónico, todas sus variaciones
conocidas, las entidades involucradas y las restricciones regulatorias.
El objetivo es que cualquier PM pueda entender la operatoria con
precisión sin necesidad de consultar a Mesa.

## Contexto

El rulo es el servicio central del grupo: convierte assets del cliente
(ARS o USD) en stablecoins pasando por Haz Pagos (PSP), una ALyC, y
Circuit Pay (PSAV). Es el producto que articula toda la operatoria del
grupo y el origen de los límites de cliente que LEX gestiona, los
quotes que TRD registra, los movimientos que OPS procesa y la posición
que FIN.Tesorería refleja.
...
| **Circuit Pay** | PSAV (CNV) — entidad propia | Recibe títulos vía contrato PSP→PSAV; ejecuta swap USD→USDT |
...
**Paso 9 — Swap USD → USDT**
Circuit Pay ejecuta el swap USD → USDT con los dólares obtenidos de la
venta de títulos.

**Paso 10 — Entrega al cliente**
El cliente recibe los USDT en su wallet crypto. El rulo está completo.
...
**Impacto en producto**: en los quotes de TRD, el campo "Tipo de TC"
distingue MEP de CCL. Legal usa esta distinción para informes
regulatorios (ver `lex-operatoria-rulo-quotes-discovery.md`).
...
Cuando el asset de origen es USD, los pasos 3–5 del flujo base (FCI +
compra de títulos) se omiten: el pipeline arranca directamente desde la
posición en USD. El tramo Circuit Pay (pasos 6–10) permanece igual en
ambas variantes.
...
- **`lex-operatoria-rulo-quotes-discovery.md`** — visibilidad de quotes
  del rulo en LEX (REQ-82). Depende de los campos TC (MEP/CCL) y moneda
  fiat que este discovery define.
- **`trd-quotes-discovery.md`** — TRD captura los quotes de cada
  operación de rulo. Es la fuente upstream de LEX para REQ-82.
- **`entities/haz-pagos.md`** — entidad PSP, rol en pasos 1–2.
- **`entities/circuit-pay.md`** — entidad PSAV, rol en pasos 6–10.
- **`fin-discovery.md`** — FIN.Tesorería y FIN.Operatoria consumen los
  eventos del rulo para registración contable y conciliación.
...
| Fecha | Cambio |
|---|---|
| 2026-05-27 | Creación. Caso base documentado (10 pasos). Tres variaciones (MEP/CCL · USDT/USDC · ARS/USD). Restricciones operativas, marco regulatorio y detalles operativos. |
