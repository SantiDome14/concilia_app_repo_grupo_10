...
| T-03 | ¿El módulo soporta TyC diferenciados por entidad (Ardua Solutions Corp / Haz Pagos / Circuit Pay) y por jurisdicción, o un documento combinado por tipo de cliente como hoy? | **Parcialmente respondida (Manual de Onboardings, 2026-06-13):** los 6 templates de AiPrise confirman que hay al menos dos conjuntos normativos distintos: templates 1-3 firman TyC de Ardua Solutions Corp (jurisdicción Canada); templates 4-6 firman TyC de Circuit Pay / Haz Pagos (normativa BCRA/CNV). El módulo **debe** soportar documentos diferenciados por entidad — un documento combinado no es viable. La pregunta que queda abierta para Legal es si Circuit Pay y Haz Pagos comparten un documento local o tienen TyC propios separados. | Legal |
...
|---|---|---|
| 2026-06-13 | **Multi-documento confirmado por estructura de templates (fuente: Manual de Onboardings, pág. 23-24):** los 6 templates de AiPrise prueban que el modelo de TyC no es de documento único. Templates 1-3 (internacionales) → Ardua Solutions Corp. Templates 4-6 (locales) → Circuit Pay / Haz Pagos bajo normativa argentina. El CMS debe gestionar al menos 3 entidades de documento: Ardua Solutions Corp, Circuit Pay y Haz Pagos. Cierra T-03 parcialmente — queda abierto si CP y HP comparten un TyC local o tienen documentos separados. | Impacta el modelo de datos del CMS: cada documento es una entidad independiente con su propio historial de versiones. El ledger de aceptación registra versión + entidad-documento + timestamp. |
| 2026-06-13 | **Columna Template en LEX como clave natural del ledger (fuente: Manual de Onboardings, pág. 7):** el dashboard de Altas de LEX ya registra qué template de AiPrise usó cada cliente. Ese dato — combinado con el documento vigente en el momento del alta — permite construir el ledger de aceptación sin requerir captura adicional en el onboarding. El vínculo cliente → TyC ya existe implícitamente en LEX. | Simplifica el diseño del ledger: no se necesita un campo nuevo de “qué TyC firmó” — se deriva del template registrado × la versión vigente en esa fecha de alta. |
