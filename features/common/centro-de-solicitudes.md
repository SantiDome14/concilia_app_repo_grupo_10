...
**Umbrella:** Centro de Solicitudes. La UI muestra **dos categorías** (Solicitudes / Tareas) diferenciadas por label/badge. **El motor es uno solo.** La diferencia entre `type: 'solicitud'` y `type: 'tarea'` es semántica y de presentación; la mecánica (estados, transiciones, `<ClosureModal>`, audit trail, triggers, routing) es idéntica.
...
El `type` deriva del `InboxTypeConfig` y se muestra como badge en el Drawer. Los filtros de la vista permiten ver Solicitudes / Tareas / Todas.
...
| Capacidad | Detalle |
|---|---|
| Modelo canónico `Solicitud<TPayload>` | Definido en `src/types/genericos.ts`, no redefinible. Incluye `type`, `concept`, `assignee`, `recurring_definition_id` |
...
| `InboxTypeConfig` | Registry por `concept` con `type` mandatorio, `creable_manualmente`, `manual_creation_capability`, `closeActions[]`, triggers, CTAs, push, auto-archive |
...
| Filtros L3 | `type` · `concept` · `state` · `target_role` · `assignee` · "Mías" · Período · dimensiones del dominio |
...
**El mismo set canónico aplica a Solicitudes y a Tareas.** Lo que cambia entre `type` es:
...
En todos los casos: validación de `concept` en el registry del `target_app`, derivación de `type`, persistencia con `state: 'pendiente'`, ejecución de `triggers_on_create[]`, disparo de notificaciones (§ Notificaciones).
...
- **Header:** id, `concept`, `type` con badge "Solicitud"/"Tarea", estado, `owner`, `assignee` cuando aplica, SLA visual (ámbar 50–100%, rojo vencido), `due_at`.
...
- Throughput por `concept` y período (diferenciado por `type`).
...
| # | Fecha | Decisión |
|---|---|---|
| 1 | 2026-05-11 | **Umbrella "Centro de Solicitudes"** con dos categorías (Solicitudes / Tareas) diferenciadas solo por label/`type`. Motor único |
